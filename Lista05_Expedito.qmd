---
title: "ME315A_Lista05"
author: "João Vitor Expedito Silva de Oliveira"
format: pdf
editor: source
---
...
```{sql connection=con}
SELECT PNAME, ACRES, ACRES * 1.20 AS ACRES_NOVA FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, ACRES, (1.0 * ACRES) / 3 AS ACRES_AJUSTADA FROM PRESERVE
```
...
```{sql connection=con}
SELECT PNAME, FEE, (1.0 * (FEE + 50.00))/2 AS FEE_AJUSTADA FROM PRESERVE
```

4. Apresente os valores médio, máximo e mínimo de ACRES para todas as reservas naturais localizadas no Arizona.

```{sql connection=con}
SELECT ACRES, AVG(ACRES) AS MEAN_ACRES, MAX(ACRES) AS ACRES_MAX, MIN(ACRES) AS ACRES_MIN FROM PRESERVE
WHERE STATE = 'AZ'
```
...
```{sql connection=con}
SELECT PNAME, 0.02 * ACRES AS NEW_FEE FROM PRESERVE
WHERE STATE = 'AZ'
```

```{sql connection=con}
SELECT AVG(0.02 * ACRES) AS MEAN_FEE FROM PRESERVE
WHERE STATE = 'AZ'
```
...
```{sql connection=con}
SELECT ENO, ENAME, SALARY AS OLDSALARY, SALARY * 1.10 AS NEWSALARY FROM EMPLOYEE
WHERE NEWSALARY > 2000.00
```

11. Apresente a soma, a média, o valor máximo e o valor mínimo de todos os valores de SALARY.

```{sql connection=con}
SELECT SUM(SALARY) AS SUM_SALARY, AVG(SALARY) AS MEAN_SALARY, MAX(SALARY) AS MAX_SALARY, MIN(SALARY) AS MIN_SALARY FROM EMPLOYEE
```
